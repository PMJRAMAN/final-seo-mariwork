...
| DEC-056 | Product image/gallery SEO policy | DISCUSSING | Store evidence + MASTER-TODO L; owner clarifications 2026-09-25 | Product-family image policy is under discussion. Owner-confirmed commercial rule: for Mariwork fabric-color Products, the primary Product image intentionally represents the 60 ml package/variant. This is a deliberate merchandising decision and must not be overridden by SEO merely to make the primary image variant-neutral. For variable fabric-color Products, selecting a volume variant changes the displayed Product image to the image for that selected volume; SEO does not require all volume images to be shown simultaneously in the gallery when the variant-image interaction already exposes the correct selected-volume image. Gallery image count, diversity, composition and merchandising order are already fixed product/design decisions and are out of scope for SEO strategy unless a specific technical discoverability, accessibility, factual-accuracy or Search issue is evidenced. ALT remains governed by DEC-054. The page must remain factually clear about available variants and must not misrepresent the purchasable Product. Remaining DEC-056 scope is limited to SEO-relevant primary/variant image consistency and technical discoverability, not gallery redesign. |
