...
| DEC-008 | Product Title Naming Standard — sets/bundles | ACCEPTED | SR-004 SR-ST-002; bundle-component-matrix; owner approval 2026-09-25 | Accepted: Set/Bundle visible Product titles/H1s use concise purchasable identity plus genuinely identifying count/type and volume where applicable. Long composition strings such as `شامل ...` do not belong in the primary title and move to visible Product content. Brand `ماری ورک` is used only for genuinely Mariwork-branded sets. Volume may appear in Set/Bundle titles because it materially distinguishes set identity. Do not force Product codes unless a real stable code exists. Normalize digits/separators consistently and do not trigger URL/slug changes. Exact per-set wording is finalized during implementation/content QA from current YITH/Woo bundle composition evidence. SEO title relationship follows DEC-011. |
...
| DEC-033 | Static/Core WordPress Page schema policy | ACCEPTED | SYS-016; SR-005 §5; owner approval 2026-09-25 | Accepted: remove the generic Article default from WordPress `page` where semantics do not fit. Static/Core pages use role-appropriate page semantics, primarily WebPage; Magazine hub retains CollectionPage; FAQ-specific schema is governed separately by DEC-086; Stores/local entity treatment separately by DEC-087. Remove Article-derived author Person nodes from generic Static/Core pages unless independently justified. Shared Organization/WebSite references may remain only when tied to the authoritative site entities. Rank Math remains primary schema owner where supported. Roll out by representative canary and rendered JSON-LD regression validation before family-wide application. |
| DEC-034 | Article schema policy | ACCEPTED | SR-005 §7; current BlogPosting output; owner clarification/approval 2026-09-25 | Accepted: genuine Magazine Articles retain BlogPosting as the default schema type. Use factual headline, representative crawlable image, original datePublished, dateModified only for real content changes, and coherent WebPage/WebSite/Organization relationships. Current Mariwork Articles do not have individual authors; therefore author must be the authoritative Mariwork Organization entity, not a fabricated/default Person. Publisher is also the same authoritative Mariwork Organization entity. Remove Article-derived Person nodes from current Articles unless a future Article genuinely has an identified individual author. Keep Rank Math as primary schema owner and avoid duplicate BlogPosting emitters. Do not auto-add unrelated FAQ/HowTo/Video schema merely because such elements occur in an Article. Rendered structured-data validation required after rollout. |
| DEC-035 | Product variation ProductGroup/variant schema architecture | ACCEPTED | SR-004 SR-ST-004; variable-schema matrix; variant-url architecture; owner approval 2026-09-25 | Accepted: use Google's single-page ProductGroup architecture for real WooCommerce variable Products. Canonical parent remains the single ProductGroup URL; each purchasable variation is represented as a variant Product with its own factual Offer and distinct preselection URL. Volume variants use supported `size` semantics; non-volume products use their real supported variation property such as color/size. Do not fabricate SKU/GTIN/MPN. Rank Math Free remains the primary schema owner. Preferred implementation is a narrowly scoped server-side Rank Math schema filter that mutates/replaces the relevant existing Product node within Rank Math's JSON-LD graph; do NOT add a second independent Product/ProductGroup JSON-LD emitter. Rank Math PRO is not required/purchased solely for this capability. If a clean single-owner graph cannot be achieved through supported filters, stop and re-review rather than shipping duplicate schema. Canary one 30/60/250 and one 30/60 Product; validate rendered graph, variant count/identity, Offer price/currency/availability, preselection behavior, canonical, duplicate entities/scripts and Rich Results output before rollout. Regression-test after Rank Math/Woo updates. |
| DEC-036 | Bundle/set schema and visible component representation | ACCEPTED | SR-004 SR-ST-005; bundle matrix; owner clarification/approval 2026-09-25 | Accepted: every current YITH Bundle/Set is a standalone fixed-volume purchasable Product with one Offer; Bundles do not use ProductGroup/hasVariant/isVariantOf. Current catalog has no mixed-volume Bundles: a 30ml Set contains 30ml components, a 60ml Set contains 60ml components, and a 250ml Set contains 250ml components. Bundle volume is an intrinsic Product attribute, not a variation dimension. YITH/Woo composition is the canonical source of truth for component identity, quantity and selected variation. Visible component lists must reconcile with that data; long `شامل ...` lists belong in Product content, not the primary title. Component links may point to durable Products and, where useful, to the matching accepted variation-preselection URL. Bundle price/currency/availability come from the actual purchasable Bundle, not manually reconstructed component totals. IMPORTANT AJAX discoverability gate: component content must not depend on a user click for crawler discovery. The current button/AJAX implementation must be audited. Prefer server-rendering the component list in initial HTML and using the button only to show/hide it; alternatively it must be present in automatically rendered DOM without user interaction. If content is fetched only after click, refactor before treating it as Search-visible content. Verify raw HTML, rendered DOM, network behavior and Google URL Inspection/Rich Results rendering. Cross-reference sitewide JS/AJAX policy DEC-080. Rank Math remains primary Product schema owner. |
| DEC-037 | Academy/LearnDash schema owner and canary | ACCEPTED | SYS-017; SR-005 §10; owner clarification/approval 2026-09-25 | Accepted: Academy SEO/schema follows the semantic role presented to users, not the raw LearnDash post type. `/academy/` remains the Academy collection root. Current WP-32714 `راهنماهای یک دقیقه‌ای ماری‌ورک` is intentionally customized in Mariwork Core/UI as a guide collection: it presents `راهنما` rather than `درس`, uses guide-oriented UI copy/actions such as `دیدن راهنماها`, and exposes a count such as `شامل 37 راهنما`. Therefore its SEO/schema identity must remain a CollectionPage/guide collection, not Course. Its 37 child URLs are individual guide/watch pages, not lessons of one coherent curriculum. Future genuine coherent curricula (e.g. beginner, advanced, doll-painting courses) may use Course semantics and should use course-oriented visible language such as `درس` and `شامل N درس` when that matches the actual structure. Semantic/UI customization must be mirrored in SEO/schema and metadata architecture. Rank Math remains first schema owner where supported; Mariwork Core may fill unsupported Academy nodes without a second conflicting graph. Current canary: guide collection + two representative guide pages. Future first true Course requires its own canary before Course-family rollout. VideoObject remains DEC-038; breadcrumbs DEC-039. |
| DEC-038 | VideoObject eligibility/ownership policy for lessons and other videos | ACCEPTED | SR-005 §10; MASTER-TODO M; Google Video/Search docs; owner clarification/approval 2026-09-25 | Accepted: video structured data is eligibility-based, not automatic. Current guide pages are educational watch pages when the primary user purpose is watching one instructional video. The host remains WebPage and the primary video may be its VideoObject main entity when required factual metadata and fetchability are verified. Current guide pages already have dedicated video cover images and supporting text that substantially mirrors the video while emphasizing key points; this supporting text is companion educational content, not a reason to classify the page as Article. Target page pattern: descriptive H1 + concise intro/context, then the primary video/player prominently near the top using its stable cover/poster, followed by expanded key points/supporting text. Do not bury the primary video at the end of a long text block if the page is intended as a watch page. Guide CollectionPage itself is not a VideoObject. Required VideoObject facts include unique name, stable crawlable thumbnailUrl and factual uploadDate; prefer factual contentUrl, otherwise valid embedUrl, plus factual description/duration/creator where available. Do not fabricate dates/durations/identities/URLs. Mariwork-produced videos may reuse the authoritative Mariwork Organization as creator. Video/player must be discoverable in Google's rendered HTML without requiring a user click to instantiate the only video representation. Articles/Products with secondary videos are inventoried but not automatically promoted to watch-page status. Rank Math remains first owner; Mariwork Core may add unsupported video nodes only within one coherent non-duplicate graph. Complete M-program inventory and two-guide canary are required before broad rollout; validate raw/rendered HTML, cover/thumbnail, source/embed fetchability, metadata, schema, URL Inspection and Video indexing. Video sitemap limited to verified eligible watch pages; Clip/SeekToAction not default. |
| DEC-039 | Breadcrumb structured-data policy across Store/Academy/content | ACCEPTED | current output + hierarchy; owner approval 2026-09-25 | Accepted: breadcrumbs are used only where a meaningful hierarchy exists and represent the typical user/navigation hierarchy rather than mechanically mirroring URL paths. Store: Shop → durable Product Category → Product; child Set categories preserve approved hierarchy; approved volume landing pages belong under Store with exact naming deferred to DEC-116. Magazine: Magazine → durable primary Blog Category → Article. Current Academy guides: Academy → One-Minute Guides collection → Guide, consistent with DEC-037 and never misrepresented as Course/Lesson. Future genuine Courses: Academy → Course → Lesson. Generic Static/Core pages, Homepage and utility/noindex pages do not receive artificial breadcrumbs solely for schema coverage. Multi-parent pages use one deliberate stable primary hierarchy. Visible breadcrumb UI and structured data must agree. Duplicate emitters are only temporarily acceptable when they express the exact same factual trail; conflicting trails must be eliminated. Validate representative Store, Magazine, Guide Collection/Guide and future Course templates in rendered output and Rich Results testing. |
...
| DEC-041 | Exact mandatory parent/hub links by family | ACCEPTED | INTERNAL-LINK-REQUIREMENT-MATRIX-DRAFT; owner approval 2026-09-25 | Accepted: each durable family must maintain crawlable links to its deliberate parent/hub hierarchy. Product → stable primary Product Category → Shop; child Product Category → real parent category → Shop; current Guide → One-Minute Guides collection → Academy; future Lesson → genuine Course → Academy; Article → stable primary Blog Category → Magazine. Multi-parent entities use one deliberate stable primary hierarchy rather than random taxonomy selection. The relationship may be supplied by breadcrumb/navigation/taxonomy UI or another appropriate crawlable component and does not require forced boilerplate links inside body copy. Cross-family educational/commercial links are excluded from this Decision and remain DEC-042–DEC-046. |
| DEC-042 | Product → Academy/Article rules | ACCEPTED | SR-004 SR-ST-009; current graph; owner approval 2026-09-25 | Accepted: Product → Academy/Article links are added only when the destination directly helps the user select, prepare, use, apply, fix/cure, care for, or otherwise correctly work with that Product or Product family. Relevant relationships are classified under DEC-040 as CORE/MANDATORY, FAMILY-SPECIFIC, or CONTEXTUAL/OPTIONAL. Broad topical similarity alone does not justify a Product-page link, and unrelated educational links must not be mass-added merely to increase internal-link count. Exact approved destinations are maintained in the Internal Link Requirement Matrix. |
| DEC-043 | Academy/Lesson → Product/Category rules | ACCEPTED | SR-004 SR-ST-009; owner approval 2026-09-25 | Accepted: Academy/Guide/Lesson → Product/Category links are used only when the Product, material, tool, or Product family is actually used, required, or provides a direct useful next step for completing the instruction. Prefer the specific Product when the instruction concerns a specific item; prefer the relevant Product Category when the user reasonably needs to choose among a family of suitable products. Do not add generic or unrelated commercial links merely for SEO/internal-link volume. |
| DEC-044 | Article → Product/Category rules | ACCEPTED | SR-005 §17; owner approval 2026-09-25 | Accepted: Article → Product/Category links are used only when the commercial destination directly supports the article topic and provides a useful next step for the reader. Prefer a specific Product when the article materially discusses or uses that item; prefer the relevant Product Category when the reader reasonably needs to choose among a product family. There is no sitewide requirement for every Article to contain a commercial link. Do not add unrelated or generic Shop/Product links merely to increase internal-link volume or SEO coverage. |
| DEC-045 | Article → Article / Article → Academy relationship | ACCEPTED | SR-005 §17; owner approval 2026-09-25 | Accepted: Article → Article/Academy links are based on semantic relevance and a useful reader next step. Valid relationships include genuine continuation, deeper detail, prerequisite/background, complementary reference, or a practical Academy guide that lets the reader apply the topic. No fixed sitewide quota requires every Article to contain a specific number of Article or Academy links. Related-content modules must not create random links solely from shared Category/Tag membership; their destinations must satisfy the same relevance rule. |
| DEC-046 | Category → education/reference links | ACCEPTED | SR-004/SR-005; owner approval 2026-09-25 | Accepted: Product Category archives are not required to add Academy/Article links while they remain product-listing-only archives. Category → education/reference links become appropriate when a Category is deliberately developed into a content-supported landing page. In that state, selected Academy/Article destinations must directly support understanding, selection, preparation, use, technique, or care for that Product Category. Do not add generic sitewide education/reference link sets solely for internal-link coverage. The content structure and requirements for Category landing pages require a separate content decision before implementation. |
| DEC-047 | Anchor-text policy | ACCEPTED | CONTENT-RESEARCH-SPEC; Page+Query limitation; owner approval 2026-09-25 | Accepted: Internal-link anchor text must be concise, natural, descriptive, and contextually accurate to the destination. Exact-match keyword repetition is not required, and a fixed anchor must not be forced across all source pages. Do not invent query-target anchors without supporting evidence. Generic anchors such as “click here” should be avoided when a meaningful descriptive phrase is naturally available. Natural anchor variation is allowed when each variant accurately describes the destination in context. |
| DEC-048 | Orphan/underlinked thresholds and approved exceptions | ACCEPTED | current graph; owner approval 2026-09-25 | Accepted: Important indexable pages with an assigned SEO/IA role must have a meaningful crawlable path from the internal site architecture. A page is orphaned when no meaningful internal crawl path reaches it. A page is underlinked when required relationships defined for its family or in the approved internal-link rules/matrix are missing. Do not use a universal minimum inbound-link count as the definition of adequate linking. Utility, noindex, temporary, or deliberately non-search-target pages may be documented exceptions. Broken URLs/404s are a separate issue from orphan/underlinked coverage. |
...
| DEC-050 | Product vs Category vs Article query/content role boundary | ACCEPTED | CONTENT-ARCHITECTURE; SR-004; owner approval 2026-09-25 | Accepted: Product pages own specific-product intent; Product Category/landing pages own product-family, browse, and choice intent; Magazine Articles own informational/editorial intent where the user primarily needs to understand, explore, compare, or read rather than browse a product family. Avoid assigning substantially the same primary intent to Product, Category, and Article pages without a deliberate reason. Ambiguous cases require later keyword/SERP research and explicit page ownership. This decision defines the role boundary only; research methodology, ownership mapping, conflict resolution, and implementation are governed by later decisions and must not be inferred or executed from DEC-050 alone. |
| DEC-051 | Magazine hub and Blog Category hierarchy | ACCEPTED | WP-70; categories 71/142; owner approval 2026-09-25 | Accepted: /mag/ is the primary Magazine hub and discovery surface for Mariwork editorial content. It provides structured access to Magazine categories and their content through category blocks, archive/list sections, latest-content modules, or other deliberate discovery components. Magazine categories form the navigational/content hierarchy beneath the Magazine hub. DEC-051 does not define the final category taxonomy, category names, or category-specific content strategy; those require separate content decisions. |
| DEC-052 | History / Artists content strategy | ACCEPTED | DEC-028; legacy map; category state; current census; owner approval 2026-09-25 | Accepted: Historical and globally significant artists may be covered as Magazine Articles when they have genuine editorial, artistic, or historical value. History content about art, fabric painting, hand printing, techniques, movements, and relevant artists belongs to the Magazine editorial architecture. These Magazine Articles are distinct from the Mariwork Artists project and must not be treated as Artist profiles merely because they discuss an artist. The future /artists/ hub and its profiles represent the Mariwork Artists directory/project. Historical Artist URL migration, one-to-one identity matching where possible, fallback to the Artists hub where no matching profile exists, and consolidation of the old Artists archive remain governed by DEC-028. DEC-052 does not define which artists must be covered, publishing volume, or a detailed History editorial calendar; those remain later content-strategy decisions. |
| DEC-053 | Canonical Mariwork facts and naming-consistency registry | ACCEPTED | BRAND-ENTITY-SEARCH; owner-confirmed fact review 2026-09-25 | Accepted: use the owner-confirmed canonical facts registry in `strategy/BRAND-ENTITY-SEARCH.md` as the factual baseline for Mariwork naming/entity/content/schema work. Canonical names: `ماری‌ورک` and `Mariwork`; founder `مریم مختاری` / `Maryam Mokhtari`; start year 1395 SH; owner-confirmed 10+ years experience; Iranian specialist brand/business in fabric colors and specialist fabric-painting/hand-printing products; official website `mariwork.ir`; verified manufacturing/formulation/audience/education/product-ownership/sales-channel boundaries recorded in the registry. Do not strengthen, infer or invent legal, product, location, social, contact or technical claims beyond the verified registry. Exact official social/contact details remain deferred to DEC-127. |

# Session 8 — Images and video

| ID | Decision | Status | Primary evidence | Blocking / notes |
|---|---|---|---|---|
| DEC-054 | ALT policy by image role | ACCEPTED | SYS-015; IMAGE-ALT-AUDIT-SUMMARY; Google Image SEO Best Practices; owner approval 2026-09-25 | Accepted: ALT is role- and context-based, not missing-count-based. Informative images require concise, factual, context-appropriate ALT; truly decorative images use empty ALT. Store/Product imagery and Academy instructional/explanatory imagery generally default to informative unless genuinely decorative. Reused image files may use different ALT across pages when page context/image role differs, provided each ALT remains factually accurate. Magazine/Article meaningful images and meaningful featured/hero images require ALT; decorative textures/separators/ornament use empty ALT; historical/artwork/artist identities must not be inferred. Homepage/Static/Category meaningful images require ALT; decorative backgrounds/patterns/shapes/UI ornament use empty ALT; primary logo ALT is concise brand identity; redundant icons beside equivalent visible text are generally decorative. Linked-image ALT must make image/link purpose understandable when the image is the meaningful link content without keyword inflation. Text-bearing images, infographics and instructional screenshots use concise purpose/meaning-oriented ALT; essential information should also exist in surrounding HTML/caption where appropriate. ALT may be generated automatically from verified structured facts when image role is deterministic (for example canonical Product or Category identity), but context-dependent Article/Academy/screenshots/artwork imagery requires contextual judgment. Automation must never infer unsupported visual facts. No keyword stuffing. Product-gallery-specific behavior remains DEC-056. |
...
| DEC-056 | Product image/gallery SEO policy | ACCEPTED | Store evidence + MASTER-TODO L; owner approvals 2026-09-25 | Accepted: for Mariwork fabric-color Products, the primary Product image intentionally represents the 60 ml package/variant as a deliberate merchandising decision and SEO must not override it merely to make the image variant-neutral. For variable fabric-color Products, selecting a volume variant changes the displayed Product image to the image for that selected volume; SEO does not require all volume images to be shown simultaneously when this interaction exposes the correct selected-volume image. Gallery image count, diversity, composition and merchandising order are fixed product/design decisions and are outside SEO strategy unless a specific technical discoverability, accessibility, factual-accuracy or Search issue is evidenced. ALT remains governed by DEC-054. Product image behavior must remain factually aligned with available variants and must not misrepresent the purchasable Product. SEO scope is limited to primary/variant image consistency and technical discoverability, not gallery redesign. |
| DEC-057 | Article/Academy instructional image policy | ACCEPTED | SYS-010/011; IMAGE-ALT-AUDIT-SUMMARY; DEC-054; owner approval 2026-09-25 | Accepted: there is no fixed SEO image quota for Articles or Academy content. Academy is video-first; supplementary images are used only when they materially improve explanation of a step, tool, result, detail or concept. Magazine/Article images are used when they improve understanding, documentation, artwork/artist/Product identification, technique illustration or editorial quality. Do not add images merely so a page 'has images for SEO'. Instructional images must align with the actual lesson/article/video context rather than generic filler. A representative Featured Image may support an Article, but multiple in-body images are not an SEO requirement. ALT remains governed by DEC-054. |
...
| DEC-060 | Shop pagination and legacy `product-page` policy | ACCEPTED | parameter evidence; owner approval 2026-09-25 | Accepted: `/shop/page/N/` remains the canonical Shop pagination architecture. Legacy/alternate `?product-page=N` states must stop being generated and should consolidate to the corresponding canonical `/shop/page/N/` URL through the approved redirect/canonical implementation path. Do not maintain two parallel pagination URL systems for the same Shop result pages. This decision does not change Product, `pa_volume`, facet/sort/display or Product-variant URL behavior. |
| DEC-061 | Product search URL policy | ACCEPTED | SR-004; owner approval 2026-09-25 | Accepted: internal Product search result URLs remain user-facing utility pages but are not independent SEO landing pages. Keep them `noindex`, exclude them from XML sitemaps and do not optimize them as query-target pages. Search functionality remains available to users. Do not rely on robots.txt blocking in a way that prevents crawlers from seeing the `noindex` directive. Product, Product Category, approved `pa_volume` and other durable landing pages remain the intended Search targets. |
| DEC-062 | Author/date archive public/indexability policy | ACCEPTED | SYSTEM-URL-SPACES-CLOSURE; DEC-034; owner approval 2026-09-25 | Accepted: Author and Date archives are not independent SEO landing-page families for Mariwork. If publicly generated, keep them out of XML sitemaps and prevent them from becoming Search targets through the approved noindex/non-public implementation path. Current Magazine Articles and durable Blog Categories remain the primary editorial discovery/index surfaces. Current Articles use the Mariwork Organization rather than individual author entities under DEC-034, so Author archives do not receive separate SEO ownership. Date archives similarly do not receive independent Search targeting merely because WordPress can generate them. Exact route existence must be verified during implementation; do not invent archive URLs from this decision. |
| DEC-063 | Legacy URL prioritization rule | ACCEPTED | SR-005 §19; A-012; owner approval 2026-09-25 | Accepted: prioritize Legacy URLs with meaningful historical evidence/value for earlier migration review, including URLs with material historical GSC clicks/impressions, proven backlinks or important internal links where evidence exists, identifiable high-value Product/Article/Artist entities, and valuable URLs currently affected by 404s, incorrect redirects or redirect chains. Low-evidence/unknown historical URLs receive lower review priority. Historical age, slug similarity, old traffic or redirect existence alone never proves semantic identity or authorizes a redirect; each mapping still requires evidence that the destination is a genuine semantic successor. |
| DEC-064 | Legacy product-volume mapping disposition | ACCEPTED | legacy-volume-url-map; owner approval 2026-09-25 | Accepted: when a historical volume-specific Product URL is proven to represent the same current variable Product and the historical volume still exists as a current purchasable variant, migrate that legacy URL to the current Product with the same volume preselected. Do not infer identity from slug similarity alone. If the historical volume no longer exists, do not fabricate variant preselection; review the URL separately. UNKNOWN mappings remain blocked from bulk redirect decisions until identity is verified. Current evidence already shows many legacy volume URLs 301 to current parent Products, but redirect ownership (including possible Nginx ownership) and same-volume preselection are not yet proven; implementation must verify existing transport first and preserve/correct it rather than recreate migration blindly. |
| DEC-065 | Historical Education → Academy mappings | ACCEPTED | education-academy-migration-map; owner confirmation 2026-09-25 | Accepted: the existing one-to-one 301 mappings from historical `/education/.../` lesson URLs to their corresponding current Academy lesson URLs are the intended target state and should be preserved when the content identity is the same. Current evidence already shows 36 such redirects; this is therefore primarily a verify-and-preserve migration rule, not a request to rebuild the migration. Redirect ownership must still be verified before implementation changes. The 5 remaining UNKNOWN archive/feed URLs are not Lesson mappings and require separate disposition; do not bulk-map pagination/feed URLs to individual Academy lessons or infer destinations from slug similarity alone. |
| DEC-066 | Historical Artist URL disposition timing | ACCEPTED | A-012 legacy map; DEC-028/052; owner decision 2026-09-25 | Accepted: defer final disposition of historical Artist URLs until the new `/artists/` system is public and stable. After launch, review historical Artist URLs manually and decide per URL whether a verified semantic successor justifies a 301 or whether the old URL should remain/return a proper 404. Do not precommit unmatched Artist URLs to the Artists hub. Do not infer identity from slug/name similarity. This supersedes only the historical-Artist redirect fallback previously stated in DEC-028 and referenced by DEC-052; all other accepted Artist architecture/content-role decisions remain unchanged. |
| DEC-067 | Redirect technical owner and implementation path | ACCEPTED | SEO-OWNERSHIP; legacy workstreams; owner decision 2026-09-25 | Accepted project architecture: Nginx is the canonical technical owner for Mariwork HTTP redirects, including migration/legacy redirects. Existing redirect rules already implemented at Nginx should remain there when correct. New SEO/migration redirects should be implemented in the centralized, documented Nginx redirect layer unless a narrowly documented exception is approved. Rank Math must not create a second redirect layer for the same URL space. Before any redirect change, audit the current Nginx rule and verify source, destination, status code and conflict/chain behavior. Performance preference is an owner/project architecture rationale, not a claimed Google ranking factor. |
...
| DEC-128 | Other removed-content legacy URL disposition | PROPOSED | A-012 legacy map; DEC-063 | Decide verified semantic redirect vs proper 404/410 for non-Artist removed content; no generic fallback redirects |
