...
| DEC-037 | Academy/LearnDash schema owner and canary | DISCUSSING | SYS-017; SR-005 §10; owner clarification 2026-09-25 | Revised proposal: schema follows semantic role, not LearnDash post type. `/academy/` remains the Academy collection root. Current WP-32714 `راهنماهای یک دقیقه‌ای ماری‌ورک` is a broad collection of short guides around fabric painting/printing/techniques, not one coherent curriculum/course, so target it as CollectionPage (not Course) and treat its 37 guide URLs as individual lesson/watch pages. Future genuine coherent curricula such as beginner, advanced, or doll-painting courses may use Course semantics only when their actual structure/content supports it; Google Course-list eligibility must be checked separately at launch and must not be assumed merely from LearnDash type. Course detail architecture should allow a WebPage with a Course main entity/provider relationship while guide collections remain CollectionPage. Rank Math remains first owner; Mariwork Core may fill unsupported Academy nodes without a second conflicting graph. Canary must include current guide collection + two representative guide lessons; a future true Course gets its own separate canary before Course-family rollout. VideoObject remains governed by DEC-038; breadcrumbs by DEC-039. |
