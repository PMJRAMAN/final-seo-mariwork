...
| DEC-019 | Academy course/lesson meta-description rule | ACCEPTED | current course/lesson output | Accepted: Course/Lesson metas are page-specific, not shared generic templates or raw `%excerpt%`. Course metas summarize educational scope/purpose; Lesson metas describe the specific technique/task/concept/learning outcome. Classify as KEEP / REFINE / REWRITE / FACT-CHECK REQUIRED. Resolve duplicates/mismatches, avoid mechanical brand/keyword injection and fixed 150/160-character rules, and verify technical/safety/performance claims before approval. Visible content remains DEC-024; video/schema remains separate. |
| DEC-020 | Product-page visible content architecture by family | DISCUSSING | CONTENT-ARCHITECTURE; product dossiers | Decide required facts/modules for colors, sets, mediums, tools |
