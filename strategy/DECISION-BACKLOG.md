...
| DEC-028 | Blog Categories lifecycle | DISCUSSING | SR-005 §8; current-blog-category-matrix; owner clarification 2026-09-25 | Revised proposal: Educational Articles (71) and History (142) remain durable Magazine hubs. `Artists` (18) is NOT an ordinary empty category: it is transitional predecessor infrastructure for an Artist project currently in development. Do not delete or map it as an unrelated empty archive. Historical/current Artist-category and Artist-page URLs that are true predecessors should consolidate to the new public Artists landing page once that destination is launched and stable; do not redirect to a draft/non-public target before launch. Other empty Blog Categories remain decommission candidates subject to semantic URL disposition. |
